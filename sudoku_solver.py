...
def index():
    if request.method == 'POST':
        ...
        for i in range(9):
            for j in range(9):
                if sudoku_start[i][j] == 0:
                    found_data.append([i+1,j+1])

        start_time = time.time()
        sudoku_starter = sudoku_start.astype(int)
        result, empty = sudoku_solver_4(sudoku_starter)
        end_time = time.time()
        ...
        found_data = [["" for _ in range(9)] for _ in range(9)]
        print = ""

    return render_template('index.html', result=result, found_data=found_data, print=print)
